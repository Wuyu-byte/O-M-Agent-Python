...
async def load_mcp_tools_safe(
    ...
    try:
        ...
        aggregate_error = format_exception_chain(e)
        logger.warning("批量加载 MCP 工具失败，尝试按服务逐个降级加载:\n{}", aggregate_error)

    tools: list[Union[BaseTool, Any]] = []
    errors: list[str] = []
    for name, server in DEFAULT_MCP_SERVERS.items():
        try:
            single_client = await get_mcp_client(
                servers={name: server},
                force_new=True,
            )
            server_tools = await single_client.get_tools()
            tools.extend(server_tools)
            logger.info("MCP 服务 [{}] 加载成功: {} 个工具", name, len(server_tools))
        except BaseException as single_exc:
            detail = format_exception_chain(single_exc)
            errors.append(f"[{name}] {detail}")
            logger.warning("MCP 服务 [{}] 加载失败，将跳过:\n{}", name, detail)

    if tools:
        err_text = "\n".join(errors) if errors else None
        return tools, err_text
    return [], "\n".join(errors) if errors else aggregate_error
...
def _create_mcp_client(
    ...
    """
    servers = _filter_available_stdio_servers(servers)
...
def _filter_available_stdio_servers(
    servers: Dict[str, Dict[str, Any]],
) -> Dict[str, Dict[str, Any]]:
    """跳过命令不存在的 stdio MCP，避免本地缺 uv 时拖垮全部 MCP 工具。"""
    available: Dict[str, Dict[str, Any]] = {}
    for name, server in servers.items():
        transport = str(server.get("transport", "")).replace("_", "-")
        command = str(server.get("command", ""))
        if transport == "stdio" and command and shutil.which(command) is None:
            logger.warning("MCP 服务 [{}] 已跳过：命令不存在 {}", name, command)
            continue
